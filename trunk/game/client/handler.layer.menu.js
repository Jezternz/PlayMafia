	...
	this.friends_callback = function(data){
		...
		if(data.friend_updates && data.friend_updates.length > 0){
			data.friend_updates.forEach(function(fUpdate){
				...
				{
					case 'friend_add':
						app.stateGlobal.friends.push({'userid':+fUpdate.userid, 'username':fUpdate.username, 'status':parseInt(fUpdate.status)});
						app.addUser(+fUpdate.userid, fUpdate.username);
					break;
					case 'friend_status':
