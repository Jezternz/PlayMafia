	...
	window.MControls = (function(){
		...
		var 
			...
			mafDialogContainerClone = $(
				'<div class="mafpopup animation_on hidden animated">'+
			...
			mafDialogButtonClone = $('<input type="submit" value="" class="mafbutton"/>'),
		...
		var MafAlert = function(obj){
			...
			{
				var buttonVal = $(e.target).is('.mafbutton') ? $(e.target).data('button-value') : container.find('.mafbutton').last().data('button-value');
				e.preventDefault();
				...
				setTimeout(function(){
					...
					callbackTimer(function(){
						if(obj.callback)obj.callback.call({}, buttonVal);
					});
				},1000);
			}
			
			if(!obj.buttons){
				var okbutton = mafDialogButtonClone.clone();
				okbutton.val('Ok');
				okbutton.data('button-value', -1);
				container.find('.foot').append(okbutton);
			} else {
				obj.buttons.forEach(function(b){
					var button = mafDialogButtonClone.clone();
					button.val(b.display).data('button-value', b.value);
					button.click(complete);
					container.find('.foot').append(button);
				});
			}
            ...
            container.find('.mafbutton').last().focus();
		}
		// Mafia Confirm box
		var MafConfirm = function(obj){
			...
			var container = mafDialogContainerClone.clone();
			var okbutton = mafDialogButtonClone.clone();
			var cancelbutton = mafDialogButtonClone.clone();
			
			okbutton.addClass('maf_ok').val('Ok');
			cancelbutton.addClass('maf_cancel').val('Cancel');
			container.find('.dialog').addClass('alert');
			container.find('.dialog').prepend('<div class="head"></div>');
			container.find('.dialog').append('<div class="foot"></div>');
			container.find('.body').append(obj.body || "");
		...
		var MafPrompt = function(obj){
			...
			var complete = function(e)
			{
				...
				setTimeout(function(){
					...
					container.remove();
					callbackTimer(function(){
						if(obj.callback)obj.callback.call({}, promptInput.val(), buttonVal);
					});
				},1000);
			}
