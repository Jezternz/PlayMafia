...
new function(){
	...
	var pageSetups = {
		...
		"settings":function(){
			...
			var setupSettingsDisplay = function(){
				...
				app.stateGlobal.settingsSetup.settings.forEach(function(setting){
					var tsetup, group = settingsStructure.groups.filter(function(g){return setting.group == g.id;})[0];
					tsetup = {
						'type':'text',
						'id':setting.id,
			...
			this.updateUserSettingsDisplay = function(){
				var newVals = {};
				app.stateGlobal.settingsSetup.settings.forEach(function(setting){
					if(typeof app.stateGlobal.usersettings[setting.id] == 'undefined')return;
		...
		"listing":function(){
			...
			var selectGameButton = function(e){
				...
				if(inputDisabled)return;
				var $target = $(e.target);
				if($target.is('.body_inner'))return;
				var newTarget = $target.closest('.row').data('gameid');
			...
			var formatSettings = function(settings){				
				...
				app.stateGlobal.gameConfig.roles.forEach(function(role){
					if(typeof vHash[role.roleid] != 'undefined'){
						...
						settingsDisplay += '<div class="listing_setting_rolecount" title="'+ lang('roles.'+role.roleid+'.name') +'">'+ vHash[role.roleid] +
		...
		"lobby":function(){
			...
			var readyLobbyButton = function(){
				$('#menu_mafia #button_lobby_ready').attr('disabled', true);
			...
			this.playerReadyChange = function(){
				$('#menu_mafia #button_lobby_ready').attr('disabled', false);
	...
	var friendsManager = new function(){
		...
		var constructor = function(){
			...
			$('#menu_mafia #chat_footer').appendTo('#mafia_container #main_overlay');
			$('#chat_footer').hide();
			$('#chat_footer #friends_listbox').hide();	
			$('#chat_footer #button_home')
			...
			$('#chat_footer #chatbox_head_close').click(closeChatTab);
		}		
		...
		var focusChatTab = function(friendUserId){
			...
			$('#chat_footer #chatbox_head_text').html('Chat: '+friend.username);
		...
		var initFriendList = function(){
			...
			if(app.stateGlobal.friends.length == 0){
				...
				app.stateGlobal.friends.forEach(addFriend);
			}
		}
		...
		var chatTabPress = function(e){
			...
			if(targetButton.size() != 0){
				var userid = parseInt(targetButton.data('userid'));
				if(e.which==1)focusChatTab(userid);
	...
	var constructor = function(){
		...
		var timg = new Image();
		timg.onerror = timg.onload = function(){
			timg = null;
		}
		timg.src = "images/mafia_images.png";
		...
		if(init!==false){
			...
			$('#menu_container, #game_container')
				.on('click.gameeventtext', function(e){
					if($(e.target).hasClass('mafia_menu_evented_text')){
						e.contextMenu = false;
